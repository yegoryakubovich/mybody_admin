...
blueprint_language_delete = Blueprint(
    name='blueprint_language_delete',
    import_name=__name__,
    url_prefix='/delete'
)


@blueprint_language_delete.route(rule='/', endpoint='delete', methods=['GET', 'POST'])
@admin_get(not_return=True)
def language_delete(language_id):
    language = Language.get_by_id(language_id)
    interface = interface_language_delete(language=language)
    return interface
